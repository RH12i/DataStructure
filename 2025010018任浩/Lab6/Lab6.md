...
| `p->data` | 25 | 25 | -1 |
| `p->next` 的地址 | 0x???????? | 0x???????? | 0x???????? |
| `prev->next` 的地址（图 C 填 `list->head->next`） | 0x???????? | 0x???????? | 0x???????? |
| `list->size` | 5 | 5 | 6 |
...
> `p` 结点的 16 个字节没有变化，因为插入只改前驱的 `next`，不改 `p` 本身。`prev` 结点的 16 个字节里只有后 8 个字节（即 `next` 成员）变了，从原来指向 42 所在的结点改成指向 `p`，前 4 字节的 `data`（-1）和中间 4 字节的对齐填充都不变。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> 不会重复释放：`unlinkAfter` 已经把 `p` 摘出链表，`head->next` 改指向 18 所在的结点，之后链表的遍历、打印和销毁都不会再经过 `p`，25 所在的结点只被释放一次，`p->next` 里的残留地址只是旧内容，不代表仍在链上。如果删掉 `free(p)`，程序输出不会变（链上结点的个数和数据没变），但被摘下的 -1 结点所占的堆内存永远不会归还，造成内存泄漏。
...
> 链表"快"在拿到前驱后插入与删除只改指针、是 `O(1)`，按秩访问要先从哨兵一步一步走所以是 `O(n)`；向量"快"在下标随机访问是 `O(1)`（有动态扩容时末尾追加均摊 `O(1)`），"慢"在中间或开头插入要整体移动元素。适合选链表的场合：元素个数无法预知、经常在已经拿到指针的位置做插入删除、不需要按下标随机访问，例如约瑟夫问题这类需要频繁从中间摘结点的问题。
